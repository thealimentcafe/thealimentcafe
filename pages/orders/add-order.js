...
function AddOrder() {
  ...
  const { fields, append, prepend, remove, swap, move, insert, replace } = useFieldArray({ control, name: "menu_items" });
  ...
  const [value, setValue2] = React.useState('cash');
  ...
  const onSubmit = data => {
    ...
    axios.post(process.env.apiUrl+'v1/orders',payLoad)
    .then(res => {
        ...
        <div className="Body">
          <div className="Container">
            ...
            <div className={`${styles.BodyHeadArea}`}>
              <Link href="/orders/list" className={`${styles.BackBU}`}><ArrowLeft/></Link>
