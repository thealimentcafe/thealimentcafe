...
function AddItem() {
  ...
  const addItem = () =>{
    if(addData !== ''){
      axios.post(process.env.apiUrl+'v1/items',{item_name:addData})
      .then(res => {
        setAddData('');
        let newState = state;
  ...
  const editItem = (id,index)=>{
    ...
    axios.put(process.env.apiUrl+'v1/items/'+id,{item_name:item_name})
      .then(res => {
        let newState = state;
        ...
        <div className="Body">
          <div className="Container">
            ...
            <div className={`${styles.BodyHeadArea}`}>
              <Link href="/stocks/list" className={`${styles.BackBU}`}><ArrowLeft/></Link>
